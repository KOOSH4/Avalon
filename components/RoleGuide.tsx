...
function getExtraInfo(role: Role) {
    const extras: Record<string, {strategy: string, question: string}> = {
        ...
        [Role.Percival]: { strategy: "وظیفه شما فداکاری است. اگر لازم شد، طوری بازی کنید که آدمکش فکر کند شما مرلین هستید.", question: "مرلین واقعی معمولاً با احتیاط بیشتری صحبت می‌کند، مورگانا سعی دارد خیلی سریع اعتماد شما را جلب کند." },
        [Role.Morgana]: { strategy: "شما باید پرسیوال را قانع کنید که مرلین هستید. اگر او به شما اعتماد کند، نیمی از راه پیروزی را رفته‌ای.", question: "تایید کردن یک بازیکن خوب در ابتدای بازی می‌تواند پرسیوال را به شک بیندازد که شما مرلین هستید." },
        [Role.Assassin]: { strategy: "تمام مدت به حرف‌های مرلین گوش دهید. او کیست؟ چه کسی همیشه تیم‌های درست را انتخاب می‌کند؟", question: "کسی که در لحظات حساس سکوت می‌کند اما رأی‌هایش همیشه درست است، مشکوک‌ترین فرد به مرلین بودن است." },
        [Role.Mordred]: { strategy: "چون مرلین شما را نمی‌بیند، بهترین فرصت را دارید تا وارد تیم‌های او شوید و در لحظه آخر شکست را بزنید.", question: "اعتماد مرلین به بازیکنانی که نمی‌شناسد، پاشنه آشیل اوست." },
        [Role.LoyalServant]: { strategy: "شما سپر انسانی مرلین هستید. باید با سوال پرسیدن و تحلیل رأی‌ها، جاسوس‌ها را گیج کنید.", question: "اگر ۳ بار به تیم‌های مختلف رأی مخالف دادید، احتمالاً دارید به جاسوس‌ها کمک می‌کنید." },
