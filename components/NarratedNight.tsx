...
const NarratedNight: React.FC<NarratedNightProps> = ({ gameState, finishNarration }) => {
  ...
  const steps = useMemo(() => {
    ...
    if (evilExceptOberon.length > 1) {
      const oberonWarning = hasOberon ? ' (به جز اوبرون)' : '';
      s.push({ 
        text: `شروران${oberonWarning} چشمانشان را باز کنند و یکدیگر را بشناسند.`, 
        subText: hasOberon ? 'اوبرون باید چشم‌هایش بسته بماند.' : undefined,
        emoji: '😈',
        color: 'text-red-400'
      });
      s.push({ 
        text: 'شروران چشم‌ها را ببندید.', 
        emoji: '🙈',
        color: 'text-red-300'
      });
    }
    ...
    if (hasLovers) {
      s.push({ 
        text: 'تریستان و ایزولت چشم‌ها را باز کنند و یکدیگر را بشناسند.', 
        emoji: '💑',
        color: 'text-blue-400'
      });
      s.push({ 
        text: 'تریستان و ایزولت چشم‌ها را ببندید.', 
        emoji: '🙈',
        color: 'text-blue-300'
      });
    }
    ...
    // 4. Merlin
    if (hasMerlin) {
      const mordredWarning = hasMordred ? ' (به جز موردرد)' : '';
      s.push({ 
        text: `شروران${mordredWarning} شست‌های خود را بالا بیاورند.`, 
        subText: hasMordred ? 'موردرد شست خود را بالا نیاورد.' : undefined,
        emoji: '👍',
        color: 'text-red-400'
      });
      s.push({ 
        text: 'مرلین بیدار شود و شروران را ببیند.', 
        subText: 'مرلین، هویت شروران را به خاطر بسپار.', 
        emoji: '🧙‍♂️',
        color: 'text-blue-400'
      });
      s.push({ 
        text: 'مرلین چشم‌ها را ببندد و شروران شست‌ها را پایین بیاورند.', 
        emoji: '🙈',
    ...
    if (hasPercival) {
      const percyText = hasMorgana 
        ? 'مرلین و مورگانا شست‌های خود را بالا بیاورند.' 
        : 'مرلین شست خود را بالا بیاورد.';
      
      s.push({ 
        text: percyText, 
        subText: hasMorgana ? 'پرسیوال نباید تشخیص دهد کدام مرلین و کدام مورگاناست.' : undefined,
        emoji: '👍',
        color: 'text-yellow-400'
      });
      s.push({ 
        text: 'پرسیوال بیدار شود و این افراد را ببیند.', 
        emoji: '🛡️',
        color: 'text-blue-400'
      });
      s.push({ 
        text: 'پرسیوال چشم‌ها را ببندد و شست‌ها پایین بیایند.', 
        emoji: '🙈',
        color: 'text-indigo-300'
      });
    }
    ...
    s.push({ 
      ...
      subText: 'صبح شده است و اولین ماموریت در شرف آغاز است!', 
      emoji: '☀️',
